...
export default function CameraSettings() {
  ...
  const updateCameraInfo = useCallback((e: FormEvent, name: string, link: string) => {
    e.preventDefault();
    ...
    if (
      ...
      cameraLink === selectedCamera?.link &&
      Number(period) === selectedCamera?.processDelay
    ) {
      setOnError(true);
      return;
    }
        ...
        <form className="camera-settings__form">
          ...
          <div>
            ...
            <input
              type="numeric"
              value={Number(period)}
              onChange={(e) => setPeriod(e.target.value.replace(/[^0-9.]/g, "").replace(/(\..*)\./g, ""))}
            />
          </div>
          <div className="camera-settings__buttons">
            <button type="submit" onClick={(e: FormEvent) => updateCameraInfo(e, cameraName, cameraLink)}>
              Сохранить
            </button>
          </div>
        </form>
