...
const CanvasSelection = () => {
  ...
  const mouseMove = function (event: MouseEvent) {
    ...
    if (canvasRef.current !== null) ctxRef.current.clearRect(1, 1, canvasRef.current.width, canvasRef.current.height);
  ...
  const draw_shapes = (ctx: any) => {
    ...
    for (let i = 0; i < shapesList.length; ++i) {
      for (let j = 0; j < shapesList[i].length; j++) {
        ...
        ctxRef.current.fill();
        if (j === shapesList[i].length - 1) ctxRef.current.lineTo(shapesList[i][0].x, shapesList[i][0].y);
        else ctxRef.current.lineTo(shapesList[i][j + 1].x, shapesList[i][j + 1].y);
      }
      ctxRef.current.stroke();
    }
    ...
    ctxRef.current.fillStyle = "yellow";
    if (connectWithStart) {
      ctxRef.current.fillRect(currentPoints[0].x - 5, currentPoints[0].y - 5, 10, 10);
    }
  };
  ...
  // add zone func
  const onSubmitAdding = useCallback(async (title: string) => {
    ...
    <>
      ...
      {endSelection && (
        <div className="canvas__button-div">
          ...
          <button className="canvas__button-item-add" onClick={confirmAdding}>
            Добавить
          </button>
          <button className="canvas__button-item-cancel" onClick={cancelAreaAdding}>
            Отмена
          </button>
        </div>
      )}
